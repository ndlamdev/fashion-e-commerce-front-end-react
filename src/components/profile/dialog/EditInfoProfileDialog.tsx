...
const EditInfoProfileDialog = () => {
	...
	const onSubmit = async (formValues: CustomerRequest) => {
		try {
			const data = {
				...formValues,
	...
	return (
		<Dialog open={dialog === 'edit-info-profile'} onOpenChange={() => dispatch(hiddenDialog())}>
			<DialogContent
				...
				<ScrollArea className={"h-100 max-sm:w-full max-sm:h-full p-5 max-md:p-2 overflow-auto overscroll-none"}>
					<DialogTitle className={"max-sm: text-black text-lg lg:text-4xl sm:text-2xl sm:text-center"}>Chỉnh sửa thông tin
						tài
						khoản</DialogTitle>
					<form onSubmit={handleSubmit(onSubmit)} className={"w-full space-y-3 p-2 max-sm:my-5"}>
						<div className="">
							<Input defaultValue={user?.full_name} placeholder={"Họ và tên của bạn"}
										 className={"rounded-lg"} {...register("full_name", {
						...
						<RadioGroup defaultValue={user?.gender} className={"flex space-x-4"} {...register("gender")}>
						...
						<div className="">
							<Input defaultValue={user?.phone} placeholder={"Số điện thoại"} className={""} {...register("phone", {
								required: true,
								pattern: {
