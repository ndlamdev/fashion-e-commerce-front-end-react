	...
	return (
		<Dialog open={open} onOpenChange={(value) => !value && showDialog("none")}>
			<DialogContent classIcon={'bg-black text-white p-5 cursor-pointer !rounded-full -translate-y-10 translate-x-10 opacity-100 '} className={"text-gray-500 sm:max-w-200 z-51"}>
				<ScrollArea className={"h-100 p-5"}>
					<DialogHeader>
						<DialogTitle className={'text-black text-4xl text-center'}>Chỉnh sửa thông tin tài khoản</DialogTitle>
					</DialogHeader>
					<form onSubmit={handleSubmit(onSubmit)} className={'space-y-3 p-2'}>
						...
						<Dialog>
							...
							<DialogContent className="w-80 h-90 p-5" classIcon={'bg-black text-white p-3 cursor-pointer !rounded-full -translate-y-10 translate-x-10 opacity-100 '}>
									<Calendar
										mode="single"
										selected={date}
										onSelect={setDate}
						...
						<div className="">
							<Input placeholder={"Số điện thoại"} className={""} {...register("phone", {	required: true,
								pattern: {
									value: /\d+/,
						...
						<Button type={'submit'} className={'p-3 text-white bg-black w-full uppercase cursor-pointer active:bg-neutral-500'}>Cập nhật tài khoản</Button>
					</form>
				</ScrollArea>
			</DialogContent>
		</Dialog>
