			...
			<div className="p-4 flex flex-wrap justify-between items-center bg-white shadow-lg rounded-lg">
				<div className=" p-4 w-2/3 bg-white">
					...
					<div className="relative w-full">
						<Progress className={" my-1 "} value={(props.levelClub + 1) * 33} />
						{levelClub.map((item: LevelType, index: number) => {
							return (
								<div key={index}
										 className={cn("absolute w-full left-0 -top-1 ", item === (levelClub.length - 1) ? `translate-x-full` : `translate-x-[${33 * index}%]`)}>
