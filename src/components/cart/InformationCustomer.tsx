	...
	return (
		<div className={`px-5 md:pb-0 lg:px-0 ${showConfirm ? "pb-30" : "pb-0"}`}>
			...
			<div className={"flex flex-col gap-4"}>
						...
						<div className={"w-full py-5 lg:w-auto lg:grow"}>
							<Select onValueChange={handleChangeProvince} value={province}>
								...
								<SelectContent>
									...
									{isErrorInfoAddresses && <p className="text-sm text-red-500">            {t("not_found_data")}
									</p>}
									{infoAddresses &&
				...
				<div>
					...
					<div className={`flex flex-col gap-3 rounded-2xl bg-gray-200 p-5 ${otherReceiver ? "block" : "hidden"}`}>
						...
						<div className={"flex gap-4"}>
							<input className={"w-full rounded-full border-1 border-gray-400 bg-white px-5 py-2 outline-none"}
			...
			<ul className={"flex flex-col gap-2"}>
				...
				<li>
					<div
						...
						<input type={"radio"} className={"h-5 w-5"} checked={payment === "ZALO_PAY" || paymentHover === "ZALO_PAY"}
									 onChange={() => {
									 }} />
				...
				<li>
					<div
						...
						<input type={"radio"} className={"h-5 w-5"} checked={payment === "PAY_OS" || paymentHover === "PAY_OS"}
									 onChange={() => {
									 }} />
			...
			<p className={"mt-2 text-[0.85rem]"}>
				{t('return_policy')}
				<a href={"#"} className={"font-bold text-blue-800 ml-1"}>
					{t('learn_more')}
				</a>
			</p>
		</div>
