	...
	return (
		<Card className={cn(`rounded-none border-0 p-2 shadow-none`, props.className)}>
				...
				<div className='absolute top-2 right-2 grid grid-cols-2 grid-rows-2'>
					{props.tags &&
						props.tags.map((tag, index) => (
							<Badge key={index} className={"lg:text-sx col-span-2 mb-2 rounded-xl border-0 bg-black text-white"}>
								{tag}
							</Badge>
						))}
					<div className=''></div>
			...
			<CardFooter className={"block p-0 text-xs font-bold sm:text-sm"}>
				<SameRadioGroup className={"flex flex-wrap items-center gap-2"} onValueChange={setColorSelected} defaultValue={colorSelected}>
					{props &&
						props.options
							...
							?.values?.map((color, index) => (
								<SameRadioGroupItem
									...
									<Label
										htmlFor={color}
										className='box-content h-full w-full rounded-sm object-center outline-2 outline-offset-2 outline-blue-700 lg:rounded-full'
									/>
								</SameRadioGroupItem>
							))}
				</SameRadioGroup>
				...
				<p className={"mb-1 flex flex-wrap gap-2"}>
					...
					{props.discount && <span className={"text-neutral-400 line-through"}>{formatCurrency(variants?.regular_price as number)}</span>}
				</p>
