					...
					<div className={"w-full"}>
						...
						<p className={"flex font-bold"}>
							<span className={"me-3 text-sm md:text-base lg:text-2xl"}>
								{formatCurrency(
									data?.data.discount && variants ? variants.regular_price * (1 - data?.data.discount.percent / 100) : (variants?.regular_price ?? 0),
								)}
							</span>
							...
							{data &&
								data?.data.options
									...
									?.values.map((color, index) => (
										<SameRadioGroupItem key={index} className={"cursor-pointer rounded-sm px-4 py-2 lg:rounded-full lg:px-6 lg:py-4"} value={color}>
										...
										return (
											<HoverCard key={index}>
												<HoverCardTrigger className={"relative"}>
													...
													<SameRadioGroupItem className={"l:h-8 absolute top-0 h-8 w-12 cursor-pointer rounded-sm lg:w-20 lg:rounded-full"} value={size}>
				...
				<div className='flex'>
					...
					<div className='border-r-1 border-gray-200'>
						<div className='p-2'>
							...
							<SameRadioGroup onValueChange={setSizeSelected} value={colorSelected} className='flex flex-wrap gap-4'>
								{getColors(data?.data.variants).map((color, index) => (
									<SameRadioGroupItem className={"rounded-sm bg-black px-6 py-4"} value={color} key={index}>
									...
									return (
										<HoverCard key={index}>
											<HoverCardTrigger className={"relative"}>
												...
												<SameRadioGroupItem className={"absolute top-0 h-10 w-12 cursor-pointer rounded-sm"} value={size}>
